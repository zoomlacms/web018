<%@ page language="C#" autoeventwireup="true" inherits="Microb_MyMicrob, App_Web_204teml0" validaterequest="false" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head runat="server">
<title>欢迎访问<%Call.Label("{$SiteName/}");%>微博-我的首页</title>
...
<div id="blk2" class="blk" style="display: none;">
	...
	<div class="main">
		...
		<ul style="margin-top: 5px; text-align: center; padding: 10px">
			...
			<input type="hidden" id="txtImgurl" runat="server" /> <br />
									...
									src="../App_Themes/Microblog/images/16.jpg" alt="" /></a><a href="javascript:void(0)">视频</a></span><span
										id="ele4"><a href="javascript:void(0)"><img src="../App_Themes/Microblog/images/17.jpg"
											alt="" /></a><a href="javascript:void(0)">音乐</a></span><span onclick="clickhuati()"><a
												href="javascript:void(0)"><img src="../App_Themes/Microblog/images/18.jpg"
		...
		<div id="r_k_main">
			...
			<div class="r_k_main_content">
				...
				<asp:Repeater ID="repeateTheme" runat="server">
					<ItemTemplate>
						   <li><p><%#GetImg(Eval("imgUrl","{0}"),Eval("ID","{0}"))%></p>
							 <div><span><a href="ThemeVBlog.aspx?id=<%#Eval("ID") %>"><font color="#1A74E2"><%#Eval("ThemeCon")%></font></a></span>
								<span><%#Eval("Title")%></span>
								<label><%#GetNewTheme(Eval("ID","{0}")) %></label></div></li>
					</ItemTemplate>
				</asp:Repeater>
					</ul>
				</div>
			...
			<div class="top_im_t">
				<div class="top_q">
					<%
						...
						{
					%>
					<b><a href="MyMicrob.aspx?uid=<%=Request.QueryString["uid"] %>">全部</a></b>
					<%
						}
						else
						{
					%>
					<a href="MyMicrob.aspx?uid=<%=Request.QueryString["uid"] %>">全部</a>
					<%
						}
					%>
					&nbsp;&nbsp;|&nbsp;&nbsp;
					<%
						...
						{
					%>
					<b><a href="MyMicrob.aspx?Type=1&uid=<%=Request.QueryString["uid"] %>">原创</a></b>
					<%
						}
						else
						{
					%>
					<a href="MyMicrob.aspx?Type=1&uid=<%=Request.QueryString["uid"] %>">原创</a>
					<%
						}
					%>&nbsp;&nbsp;|&nbsp;&nbsp;
					<%
						...
						{
					%>
					<b><a href="MyMicrob.aspx?Type=2&uid=<%=Request.QueryString["uid"] %>">图片</a></b>
					<%
						}
						else
						{
					%>
					<a href="MyMicrob.aspx?Type=2&uid=<%=Request.QueryString["uid"] %>">图片</a>
					<%
						}
					%>&nbsp;&nbsp;|&nbsp;&nbsp;
					<%
						...
						{
					%>
					<b><a href="MyMicrob.aspx?Type=3&uid=<%=Request.QueryString["uid"] %>">视频</a></b>
					<%
						}
						else
						{
					%>
					<a href="MyMicrob.aspx?Type=3&uid=<%=Request.QueryString["uid"] %>">视频</a>
					<%
						}
					%>&nbsp;&nbsp;</div>
				<div class="top_s">
					<input id="top_s_input" value="搜索关注人的话..." runat="server" onfocus="this.value=''" />
					<asp:ImageButton src="../App_Themes/Microblog/images/ti.jpg" alt="搜索" runat="server"
						ID="sear" OnClick="sear_Click" /></div></div>
		<div class="lin">
		</div>
