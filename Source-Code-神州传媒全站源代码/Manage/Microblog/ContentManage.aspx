<%@ page language="C#" autoeventwireup="true" inherits="manage_Microblog_ContentManage, App_Web_h0ze5h3z" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head runat="server">
<title>微博广播管理</title>
...
<tr class="tdbg">
    ...
    <td align="center" class="title" width="5%">类型</td>
    <td align="center" class="title" width="5%">转发数</td>
    <td align="center" class="title" width="10%">推荐级别</td>
...
<asp:Repeater ID="repter" runat="server" onitemcommand="repter_ItemCommand">
    <ItemTemplate>
        <tr class="tdbg" onmouseover="this.className='tdbgmouseover'" onmouseout="this.className='tdbg'">
            ...
            <td align="center"><%#Eval("Elect")%></td>
            <td align="center"> <asp:LinkButton ID="btnDel" runat="server" CommandName="del" CommandArgument='<%#Eval("id") %>' OnClientClick="return confirm('确定删除！')">删除</asp:LinkButton>
            <asp:LinkButton ID="btnElite" runat="server" CommandName="Elite" CommandArgument='<%#Eval("id") %>' >推荐</asp:LinkButton></td>
        </tr>
    </ItemTemplate>
</asp:Repeater>
<tr class="tdbg">
...
<tr>
    ...
    <asp:Button ID="Button1" Style="width: 110px" class="C_input" Text="批量删除" runat="server" onclick="Button1_Click" OnClientClick="if(!IsSelectedId()){alert('请选择内容');return false;}else{return confirm('确定删除？')}" />
    <asp:Button ID="Button2" Style="width: 110px" class="C_input" Text="批量推荐" runat="server" onclick="Button2_Click" OnClientClick="if(!IsSelectedId()){alert('请选择内容');return false;}else{return confirm('确定推荐？')}" />
      <asp:Button ID="Button3" Style="width: 110px" class="C_input" Text="取消推荐" runat="server" onclick="Button3_Click" OnClientClick="if(!IsSelectedId()){alert('请选择内容');return false;}else{return confirm('确定取消推荐？')}" />
    </td>
</tr>
</tbody>
</table>
