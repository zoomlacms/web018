<%@ page language="C#" autoeventwireup="true" inherits="MIS_MisSignSetInfo, App_Web_ejxx4u4s" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE html>
<html>
    ...
    <form id="form1" runat="server">
     <div id="pro_left">
       <ul class="AttenLis">
        ...
        <div class="Chkdays">
          工作日：<asp:CheckBoxList ID="cbkDays" runat="server" RepeatDirection="Horizontal">
        ...
        <div class="SetTime">
            上班时间：<input id="B_time" class="M_input" type="text"  onfocus="WdatePicker({dateFmt:'HH:mm'});" runat="server"/>下班时间：<input id="E_time" type="text" class="M_input" onfocus="WdatePicker({dateFmt:'HH:mm'});" runat="server"/>
        </div>
        <div class="BPTime">
            开始执行日期：<asp:TextBox ID="TextDate" onclick="WdatePicker({ dateFmt: 'yyyy/MM/dd HH:mm:ss' });" class="M_input" runat="server"/>
        </div>
        <div>
            <asp:Button ID="BtnSub" runat="server" OnClick="BtnSub_Click" CssClass="i_bottom" Text="提交"/>&nbsp;<asp:Button ID="BtnReset" runat="server" OnClick="BtnReset_Click" CssClass="i_bottom" Text="取消"/>
        </div>
    </div>
    </div>
    </form>
