<%@ page language="C#" autoeventwireup="true" inherits="ShoppingCart, App_Web_b0bo3n1v" enableviewstatemac="false" enableEventValidation="false" viewStateEncryptionMode="Never" %>
<!DOCTYPE HTML>
<html>
<head>
<title>加入购物车</title>
    ...
    <asp:Repeater ID="cartinfo" runat="server" OnItemDataBound="cartinfo_ItemDataBound">
      <ItemTemplate>
        <ul <%#(Eval("Bindpro","{0}")=="")?"":"style=background-color:#E6E6E6"%>>
          ...
          <li class="i6"><span id='currentpri<%#Eval("id") %>'> <%#getjiage(Eval("proclass", "{0}"), "2", DataBinder.Eval(Container, "DataItem.proid", "{0}"), DataBinder.Eval(Container, "DataItem.pronum", "{0}"))%></span></li>
          ...
          <li class="i9"><a href="UserCart.aspx?menu=del&cid=<%#Eval("id")%>" onclick="return confirm('不可恢复性删除数据,你确定将该数据删除吗？');">删除</a></li>
        <div class="clear"></div>
        </ul>
     </div>
        ...
        <asp:Repeater ID="Repeater2" runat="server">
          <ItemTemplate>
            <ul>
              ...
              <li class="i2"><%#Eval("Attribute")%></li>
              <li class="i3"><%#getProUnit(DataBinder.Eval(Container, "DataItem.id", "{0}"))%></li>
              <li class="i4"></li>
              <li class="i5"><%#getjiage(Eval("proclass", "{0}"), "1", DataBinder.Eval(Container, "DataItem.id", "{0}"), DataBinder.Eval(Container, "DataItem.pronum", "{0}"))%></li>
              <li class="i6"><%#getjiage(Eval("proclass", "{0}"), "2", DataBinder.Eval(Container, "DataItem.id", "{0}"), DataBinder.Eval(Container, "DataItem.pronum", "{0}"))%></li>
              <li class="i7"><%#getprojia(Eval("ID", "{0}"), DataBinder.Eval(Container, "DataItem.pronum", "{0}"))%></li>
              <li class="i8"><a href="UserCart.aspx?menu=del&cid=<%#Eval("id")%>" onclick="return confirm('不可恢复性删除数据,你确定将该数据删除吗？');">删除</a></li>
            <div class="clear"></div>
            </ul>
          </ItemTemplate>
        </asp:Repeater>
